...
if MIP == 'cmip5':
  ...
  if exp == 'historical': modver = 'v20200426'
...
verd= '*'
if exp == 'amip': filename_template = "%(variable)_%(model_version)_%(table)_amip_%(exp)r1i1p1_198101-200512-clim.nc"
if exp == 'amip': filename_template = "CMIP5.amip.%(model_version).r1i1p1.mon.%(variable).198101-200512.AC.v20190225.nc"
#if exp == 'historical': filename_template = "CMIP5.historical.%(model_version).r1i1p1.mon.%(variable).198101-200512.AC.v20190307.nc"
if exp == 'historical' and MIP == 'cmip5': filename_template = MIP + '.historical.%(model_version).r1i1p1.mon.%(variable).198101-200512.AC.' + modver + '.nc'
if exp == 'amip' and MIP == 'cmip5': filename_template = MIP + '.amip.%(model_version).r1i1p1.mon.%(variable).198101-200512.AC.' + modver + '.nc'
if exp == 'historical' and MIP == 'cmip6': filename_template = MIP + '.historical.%(model_version).r1i1p1f1.mon.%(variable).198101-200512.AC.' + modver + '.nc'
if exp == 'amip' and MIP == 'cmip6': filename_template = MIP + '.amip.%(model_version).r1i1p1f1.mon.%(variable).198101-200512.AC.' + modver + '.nc'


#if exp == 'historical': filename_template = "CMIP5.historical.%(model_version).r1i1p1.mon.%(variable).198101-200512.AC.%('*').nc"
if exp == 'picontrol': filename_template = "%(variable)_%(model_version)_%(table)_picontrol_%(exp)r1i1p1_01-12-clim.nc"
...
custom_observations = os.path.abspath('/p/user_pub/PCMDIobs/catalogue/pcmdiobs2_clims_catalogue_v20200420.json')
...
test_clims_interpolated_output = '/work/gleckler1/processed_data/metrics_package/pmp_diagnostics' + '/interpolated_model_clims_' + exp +'/' + case_id
test_clims_interpolated_output = '/p/user_pub/pmp/pmp_results/pmp_v1.1.2/diagnostic_results' + '/interpolated_model_clims_' + exp +'/' + case_id
...
filename_output_template = MIP + ".%(model_version)." + exp + ".r1i1p1.mo.%(variable)%(level).%(period).interpolated.%(regrid_method).%(region).AC." + case_id + "%(ext)"
